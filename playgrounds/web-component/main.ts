import "@avatar-web-component/src";
import type { AvatarElement } from "@avatar-web-component/src";
...
const sections: Array<{ title: string; style: Style<AvatarOptions>; options: AvatarOptions[] }> = [
...
if (app) {
    for (const section of sections) {
        ...
        for (let i = 0; i < section.options.length; i++) {
            ...
            el.styleImpl = section.style;
            el.options = section.options[i];
            el.setAttribute("alt", `${section.title} avatar ${i + 1}`);
            row.appendChild(el);
        }
        app.appendChild(row);
    }
