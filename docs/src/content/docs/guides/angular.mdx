...
`AvatarComponent` is standalone, so add it to the `imports` of whichever
component uses it:
...
If your application still uses NgModules, import `AvatarModule`, which
re-exports the standalone component:
