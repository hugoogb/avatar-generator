...
`@avatar-generator/core`, which knows only about `AvatarOptions`, `Style`,
`Random` and `AvatarResult`:

<Code code={`import { faces, type FacesOptions } from "@avatar-generator/style-faces";`} lang="ts"></Code>
...
These are the only types `@avatar-generator/core` declares. Everything
style-specific lives in the style's own package.
